...
const GradientMesh = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      blobs.forEach((blob) => {
        const x = blob.x + Math.sin(time * blob.speedX * 3) * 100;
        const y = blob.y + Math.cos(time * blob.speedY * 3) * 80;
    ...
    const handleResize = () => {
      ...
      blobs[0].x = width * 0.3;
      blobs[0].y = height * 0.3;
      blobs[1].x = width * 0.7;
      blobs[1].y = height * 0.6;
      blobs[2].x = width * 0.5;
      blobs[2].y = height * 0.8;
    };
