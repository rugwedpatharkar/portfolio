...
const Footer = () => {
  ...
  return (
    <footer className="glass-card-dark py-6 sm:py-8 pb-20 sm:pb-24 px-4 sm:px-6 border-t border-[#915eff]/10"
      ...
      <div className="max-w-7xl 3xl:max-w-[2000px] mx-auto flex flex-col md:flex-row justify-between items-center gap-4 sm:gap-6">
        <div className="text-center md:text-left">
          <p className="text-white font-heading font-bold text-body sm:text-body-lg">{personalInfo.fullName}</p>
          <p className="text-secondary text-caption sm:text-body-sm mt-1">{personalInfo.role}</p>
        </div>
        ...
        <ul className="flex flex-wrap justify-center gap-3 sm:gap-6">
          {navLinks.map((link) => (
            <li key={link.id}>
              <a
                href={`#${link.id}`}
                className="text-secondary hover:text-white text-caption sm:text-body-sm transition-colors link-hover-underline"
              >
                {link.title}
              </a>
            </li>
        ...
        <div className="flex gap-4">
          <a
            ...
            className="text-secondary hover:text-white text-body-lg sm:text-subheading transition-colors icon-bounce"
            aria-label="GitHub"
          >
            <AiOutlineGithub />
          </a>
