/*
 * Real planetary fact data. Distances are mean orbital, diameters in
 * km, day/year in Earth units, gravity in m/s². "Wow" is a short
 * non-obvious fact a recruiter could share at a party.
 */

export const PLANET_FACTS = {
  sol: {
    body: "Sol — G2V yellow dwarf star",
    distance: "0 AU · 8 light-min from Earth",
    diameter: "1,391,400 km · ~109 Earths",
    year: "Orbits the Milky Way core every ~230 M yr",
    day: "Surface rotates every ~25 d at the equator",
    gravity: "274 m/s² · 28× Earth",
    atmosphere: "Hydrogen (74%) · Helium (24%)",
    moons: "0 (we are one of 8 planets)",
    wow: "Loses ~4.3 million tonnes of mass per second to fusion + solar wind. Still has 5 billion years left.",
  },
  about: {
    body: "Mercury — M-class terrestrial",
    distance: "0.39 AU · 3.2 light-min",
    diameter: "4,879 km · 0.38× Earth",
    year: "88 Earth days",
    day: "59 Earth days (3:2 spin-orbit resonance)",
    gravity: "3.7 m/s² · 0.38× Earth",
    atmosphere: "Thin exosphere — sodium, helium, oxygen",
    moons: "0",
    wow: "Day-side hits 430 °C, night-side drops to −180 °C. Largest temperature swing of any solar system body.",
  },
  funfacts: {
    body: "Venus — M-class terrestrial",
    distance: "0.72 AU · 6.0 light-min",
    diameter: "12,104 km · 0.95× Earth",
    year: "225 Earth days",
    day: "243 Earth days · longer than its year",
    gravity: "8.87 m/s² · 0.90× Earth",
    atmosphere: "CO₂ (96%) · sulfuric-acid clouds",
    moons: "0",
    wow: "Spins backward (retrograde) — the sun rises in the west on Venus. Surface pressure equals 900 m underwater on Earth.",
  },
  experience: {
    body: "Earth — M-class terrestrial · cradle",
    distance: "1.00 AU · 8.3 light-min",
    diameter: "12,742 km",
    year: "365.25 days",
    day: "23h 56m 4s (sidereal)",
    gravity: "9.81 m/s²",
    atmosphere: "N₂ (78%) · O₂ (21%) · Ar (0.9%)",
    moons: "1 (Luna)",
    wow: "Only known body where water exists in all three states at the surface simultaneously. The night-side glow you see is real city lights, ~20% of which use renewable power.",
  },
  projects: {
    body: "Mars — M-class terrestrial",
    distance: "1.52 AU · 12.7 light-min",
    diameter: "6,779 km · 0.53× Earth",
    year: "687 Earth days",
    day: "24h 37m (sol)",
    gravity: "3.71 m/s²",
    atmosphere: "CO₂ (95%) · N₂ (2.8%)",
    moons: "2 (Phobos + Deimos)",
    wow: "Hosts Olympus Mons, the largest known volcano: 22 km tall (2.5× Everest), base wider than the state of Arizona.",
  },
  achievements: {
    body: "Asteroid Belt — C/S/M spectral mix",
    distance: "2.2–3.2 AU between Mars & Jupiter",
    diameter: "Ceres ~940 km · most rocks < 10 km",
    year: "Belt orbits 4.6 Earth years on average",
    day: "Tumble periods 2h–20h typical",
    gravity: "Negligible (largest: Ceres 0.27 m/s²)",
    atmosphere: "None",
    moons: "Many asteroids have their own moons",
    wow: "Total mass is only ~4% of Earth's Moon. Hollywood lied — flying through is mostly empty space (avg separation: 1 million km).",
  },
  skills: {
    body: "Jupiter — class I gas giant",
    distance: "5.20 AU · 43 light-min",
    diameter: "139,820 km · 11× Earth",
    year: "11.86 Earth years",
    day: "9h 56m (fastest spin in the system)",
    gravity: "24.79 m/s² · 2.53× Earth",
    atmosphere: "H₂ (90%) · He (10%) · ammonia ice clouds",
    moons: "95 confirmed (Io, Europa, Ganymede, Callisto)",
    wow: "The Great Red Spot is a storm wider than Earth that has raged for at least 350 years. Jupiter's magnetic field is 20,000× stronger than Earth's.",
  },
  notes: {
    body: "Saturn — class II gas giant · ringed",
    distance: "9.54 AU · 79 light-min",
    diameter: "116,460 km · 9.4× Earth",
    year: "29.45 Earth years",
    day: "10h 33m",
    gravity: "10.44 m/s² · 1.06× Earth",
    atmosphere: "H₂ (96%) · He (3%) · methane",
    moons: "146 confirmed (Titan, Enceladus, Mimas)",
    wow: "Saturn's density is less than water — it would float in a bath big enough. Its rings are 99.9% ice, only 10 m thick on average.",
  },
  education: {
    body: "Uranus — ice giant · tilted 97.8°",
    distance: "19.19 AU · 159 light-min",
    diameter: "50,724 km · 4× Earth",
    year: "84 Earth years",
    day: "17h 14m (retrograde)",
    gravity: "8.69 m/s² · 0.89× Earth",
    atmosphere: "H₂ (83%) · He (15%) · methane gives cyan tint",
    moons: "28 (Titania, Oberon, Miranda)",
    wow: "Rolls on its side — its axis tilt is 97.8°. Each pole sees 42 Earth years of continuous sunlight, then 42 years of darkness.",
  },
  hobbies: {
    body: "Neptune — ice giant · methane blue",
    distance: "30.07 AU · 250 light-min",
    diameter: "49,244 km · 3.9× Earth",
    year: "165 Earth years",
    day: "16h 6m",
    gravity: "11.15 m/s² · 1.14× Earth",
    atmosphere: "H₂ (80%) · He (19%) · methane",
    moons: "16 (Triton, Nereid)",
    wow: "Fastest winds in the solar system — supersonic, up to 2,100 km/h. Only visited once (Voyager 2 flyby, 1989). Discovered by math before telescope confirmation.",
  },
  testimonials: {
    body: "Kuiper Belt — trans-Neptunian region",
    distance: "30–50 AU",
    diameter: "Pluto ~2,377 km · most KBOs < 100 km",
    year: "Pluto orbits in 248 Earth years",
    day: "KBO rotations 4h–24h typical",
    gravity: "Negligible",
    atmosphere: "Pluto: thin N₂/CH₄ (forms when near sun)",
    moons: "Pluto has 5 (Charon, Nix, Hydra, Kerberos, Styx)",
    wow: "Pluto's largest moon Charon is so massive (half Pluto's size) the pair orbit a point in space between them — they're a true double dwarf-planet system.",
  },
  contact: {
    body: "Edge Beacon — heliopause boundary",
    distance: "~49 AU · 6.8 light-hours",
    diameter: "ø — concept marker",
    year: "—",
    day: "—",
    gravity: "—",
    atmosphere: "Interstellar medium begins here",
    moons: "—",
    wow: "Voyager 1 crossed the heliopause in 2012 at 121 AU and entered interstellar space — the first human-made object to leave the solar system.",
  },
};
